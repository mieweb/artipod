...
- **Docker Integration**: Run commands in isolated, sandboxed Docker containers
- **Security Hardening**: Seccomp profiles, read-only filesystems, resource limits
- **Per-Pod Configuration**: Each pod can use different Dockerfiles and security profiles
- **Context Mounting**: Automatically mount pod filesystem into containers
...
Blocked syscalls include: kernel module loading, system reboot, filesystem mounting, hardware access, and more.
...
interface BuildPromptOptions {
  ...
  includeFiles?: boolean;           // Include file listings (default: true)
...
Here's a minimal Dockerfile for use with ArtiPod:

```dockerfile
FROM node:18-alpine

# Create non-root user
RUN addgroup -S artipod && adduser -S artipod -G artipod

# Set up working directory
WORKDIR /context
RUN chown artipod:artipod /context
...
# Pod mounts will be available at /context/<mount-name>
CMD ["/bin/sh"]
```

## Repository & Examples

For full source code, examples, and development information, visit the [GitHub repository](https://github.com/mieweb/artipod).
