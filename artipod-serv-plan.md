...
- 2026-09-02 (addendum 2 — owner: filenames in `find /.artipod/uppers` must not be plaintext): **encryptedStoreMount** (`src/sandbox/encrypted-store.ts`) — option B: the whole upper (names, dirs, inodes, data) lives as ZenFS StoreFS blocks keyed by inode NUMBER, every block value encrypted to the inner store; backings: `opfs` (block-per-file dir, `OpfsDirStore`), any store-based mount `config` (InMemory/IndexedDB), or a raw `store`. The backing shows only numbered ciphertext blocks; demo upper dirs are named `hash(ref)` so fork existence is hidden too; plaintext cache warmed at mount (locked key ⇒ non-empty mount refused). StoreFS store-contract landmines: non-'partial' transactions receive PRE-MERGED whole values — `set(id, data, 0)` REPLACES (keeping an old tail corrupts shrunken dir listings) and `get`/`cached` return the WHOLE value (StoreFS does its own range slicing — honoring offset/end double-slices to empty); always copy out of the cache (StoreFS grows buffers via `transfer()`, detaching shared backings); cache updates must land in the SYNCHRONOUS prefix of set/delete (`AsyncTransaction.setSync` fires the async method and moves on — the next statSync must already see it) with ordered persistence queued behind. Demo catalog: legacy plaintext uppers (`encodeURIComponent` names) keep dir-contents-as-truth; hashed block dirs are opaque — physical dir only vetoes (gone/empty = unchanged), the workspace-maintained registry flag is the verdict. encryptedMount (contents-only) stays exported for name-preserving use cases. Tests: encrypted-store.test.ts (4: name+content hiding on the backing, remount incl. deletions, locked-mount refusal, config backing).
