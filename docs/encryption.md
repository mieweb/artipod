...
- **Working trees encrypt at rest too**: `encryptedMount` (from `/sandbox`) wraps any ZenFS backend in the same chunked-AEAD format for overlay uppers and scratch dirs — contents are ciphertext on OPFS/IndexedDB, names/sizes stay visible, plaintext lives only in the in-memory mirror, and a locked keyring refuses the mount.
