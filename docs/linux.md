...
> **Status: mostly ✅.** Hardened Docker execution, seccomp profile, the Node core, realizers, the OCI-layout store, and the manager all ship in this repo today (plan Phases 3–6.6); server bash sessions + git/OCI proxies ship in `@artipod/core/server`. 🔮 remains for container2wasm and ZenFS-on-disk.
...
`npm ci && npm run build && sudo systemctl restart artipod-sync`. Environment: `EXEC_API_TOKEN` (optional bearer auth for `/api/exec`), `GIT_PROXY_ALLOWED_HOSTS`, and `ARTIPOD_OCI_ALLOWED_HOSTS` (empty = proxy disabled), KMS/key-authority configuration for the manager.
