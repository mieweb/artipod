...
import type { ChatMessage } from '@artipod/core/agent';
import type { LocalModelClient } from '@artipod/core/agent';
import { CURATED_MODELS, DEFAULT_LOCAL_MODEL, type LocalModelInfo } from '@artipod/core/agent';
import { formatBytes, listCachedModels, type CachedModel } from '@artipod/core/agent';
...
function persistConfig(config: AgentConfig, rememberKey: boolean): void {
  const { apiKey: _apiKey, ...withoutKey } = config;
...
export default function AgentPanel({ getSandbox, events, getLoopOptions }: AgentPanelProps) {
  ...
  useEffect(() => {
    ...
    import('@artipod/core/agent').then(({ listLocalModels }) =>
  ...
  const getLocalClient = async (): Promise<LocalModelClient> => {
    const { LocalModelClient } = await import('@artipod/core/agent');
    const { modelInfo } = await import('@artipod/core/agent');
