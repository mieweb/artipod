...
- [x] **`app/api/exec/route.ts`** (Node runtime, not edge): per-session sandbox registry (`Map<sessionId, Sandbox>` with TTL eviction). ZenFS also runs in Node — reuse `createSandbox` with an `InMemory` ZenFS mount so **git works server-side identically** (clone in a server sandbox). POST `{ sessionId, command }` → `{ stdout, stderr, exitCode, cwd }`.
  - Set conservative `executionLimits` and consider just-bash `network` config only if server `curl` is wanted. *(`hardened` limit profile + 256 MiB `maxFileSystemBytes`; network stays off.)*
  - This is the "untrusted script author" scenario from just-bash's THREAT_MODEL — the interpreter is the sandbox; still add per-session memory caps (in-memory fs growth) and rate limiting. *(Isolation via `bindContext('/sessions/<id>')` chroot views over one global InMemory mount; per-session in-flight guard → 429; 50-session cap → 503; 30 s exec timeout; optional `EXEC_API_TOKEN` bearer auth. Registry lives in [lib/server/exec-sessions.ts](lib/server/exec-sessions.ts) — routes are thin wrappers per the script-first convention. ZenFS/just-bash must be `serverComponentsExternalPackages` — webpack's server bundle breaks ZenFS class construction.)*
- [ ] **Real-disk mode (optional, feature-flagged)**: `OverlayFs`/`ReadWriteFs` from the just-bash root entry for server workspaces on disk (reads real dir, writes CoW or direct). No isomorphic-git in this mode initially. *(Deliberately not implemented — optional, and the path allowlist design in §8 is still open.)*
- [x] **`app/api/git/[...proxy]/route.ts` (recommended)**: self-hosted CORS proxy (port of `@isomorphic-git/cors-proxy` logic) so browser git stops depending on `cors.isomorphic-git.org` (rate-limited, availability not guaranteed). Make the proxy URL configurable in [lib/git.ts](lib/git.ts); keep the public proxy as fallback for pure-static deployments. *(Host allowlist — github/gitlab/bitbucket/codeberg/gitea, overridable via `GIT_PROXY_ALLOWED_HOSTS`; smart-HTTP endpoints only; request/response header allowlists; validators in [lib/server/git-proxy.ts](lib/server/git-proxy.ts). Point the browser at it with `NEXT_PUBLIC_GIT_CORS_PROXY=/api/git`; public proxy remains the default fallback.)*
- [x] CLI parity note: for local dev, `npx just-bash-shell` already gives a Node REPL over a real dir; our route handler is for hosted/agent use.
