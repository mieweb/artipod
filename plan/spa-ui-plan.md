...
**RECOMMENDATION** (the phase's contract; consistency model ratified with the owner, 2026-09-03):

**Engine**: adopt bare kerebron `extension-yjs` (via the RichEditor `collab` prop we already ship); **defer yorm**. Zero new dependencies, the surface is our editor, the protocol is commodity y-websocket. yorm re-enters at the first *structured* collab need — MDY front-matter co-editing (typed objects, not prose) or AgentPod suggestion-mode review are its natural openings.

**The consistency model — shared working memory above the pod:**
- The Y.Doc is the room's **working memory**, living entirely ABOVE the artipod: while anyone is editing, Yjs synchronizes the participants' editors and **the disk sees nothing** — no write-back stream, no elected writer. The Y.Doc is the source of *proposed* truth for editors; the artipod is the source of truth for the *disk*.
- **Save is an explicit, credited act.** Whoever presses Save authors that head — their actor id rides the push exactly like a single-user save (per-file LWW + `org.artipod.actor`, no new authorship concept). The save intent broadcasts to the room (shared map/awareness field `saving: <actor>` → `saved`); all other participants BLOCK saving until the saver finishes. After the save, nothing special happens: participants may or may not observe the disk change underneath, and it doesn't matter — the room stays the proposed truth. Disk writes from outside the room (shell edits) likewise don't disturb it; the next save overwrites, which is LWW stated honestly.
- **Ephemeral by design, with a crash grace**: the relay keeps a room's state for **~15 minutes after the last peer disconnects** (crash/refresh recovery — rejoin inside the window and the proposal is still there), then **purges**. If the room is empty past the grace, unsaved proposals evaporate — the same semantics as closing an editor with unsaved changes. No pod-resident update log, no long-term relay persistence, and the e2e-storage question disappears with them.
- **Seeding**: first joiner of an EMPTY room seeds from the pod file; every later joiner adopts the room and never seeds (fixes finding #2's double-seed). The 15-min retention makes "empty" unambiguous relay-side.
- **UI honesty requirement**: the editor must make **"nothing is saved until it's saved"** unmistakable while collab is live — an explicit unsaved-proposal indicator (e.g. `proposal — not saved` chip with participant count), the dirty ● semantics kept truthful (dirty = room differs from disk, cleared only by an actual save), and a leave/close warning when the room holds unsaved changes. Same honesty bar as the sync badges: claim nothing the disk can't back.

**Production wiring is follow-up work** (a serve-plan phase for the relay, a consuming milestone here): ① core `serveApp` WS upgrade hook serving `/yjs` (ask-first) — and under this model the relay is *dumb*: pure message forwarding + the 15-min room retention buffer; it never integrates a doc (the very job `@y/websocket-server` was failing at is not required); ② room auth via lease-derived token, room names hashed like upper dirs (pod paths must not leak); ③ optional hardening: KEK-encrypt the updates in flight — every legitimate peer holds the leased key, the relay relays ciphertext, and the blind-host property extends to the live layer; ④ the save-barrier protocol + UI indicators above. With ①–④, "one tab at a time" retires for collab-managed files.
