...
|---|---|---|---|
| A — repo consolidation (move artipod-sync in, keep history) | `sync-a-move` | todo | |
...
- [ ] Owner go-ahead on the merge push + archive plan (ask-first)
- [ ] filter-repo import per §3.1; `git log --follow` shows pre-move history for `app/page.tsx` and `lib/server/exec-sessions.ts`
- [ ] Tidy commit (attic moves, nested `.github` folded, READMEs)
- [ ] Wiring commit: `file:../..` + `.npmrc` intact; `npm ci && npm run build && npx vitest run` green **inside `examples/artipod-sync`**
- [ ] CI `example-app` job green on the PR
- [ ] Root suite untouched: `npm test` green, package `files`/exports unchanged
